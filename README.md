...
- 最近更新：后台周期性扫描 Emby 新增影视（复用现有 worker，默认每 5 分钟），用户中心「我的订阅」页展示横向可滑动的「最近更新」海报条（海报经服务端代理加载，隐藏 API Key）；若新增条目命中待处理的求剧（kind=full），自动标记为「已入库」并展示「求剧已入库」，无需管理员手动操作。
...
**自动入库**：后台 worker 周期性扫描 Emby 最近新增的影视（默认每 5 分钟，随主 worker 运行，无需额外配置）。新增条目写入用户中心的「最近更新」区块（登录后可见，保留最近 50 条）；当某条新增命中「待处理」的求整部求剧时，系统自动把该求剧标记为「已入库」，用户可在「最近更新」卡片上看到「求剧已入库」标记。「催更」类求剧针对缺失剧集，新增剧集条目并不等于补齐缺集，因此不会自动标记，仍需管理员确认。
...
│   ├── invites.go        邀请码与兑换存取
│   ├── requests.go       求剧记录存取
│   ├── recent.go         最近更新存取（含命中求剧自动入库）
│   └── sync.go           Emby 访问同步 outbox
├── portal/               用户中心
├── requests/             求剧（TMDB 搜索与求剧记录）
├── recent/               最近更新（Emby 新增影视监测与自动入库）

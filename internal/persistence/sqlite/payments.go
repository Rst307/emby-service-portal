...
func (s *Store) CreatePaymentPlan(ctx context.Context, input domain.CreatePaymentPlanInput, now time.Time) (domain.PaymentPlan, error) {
...
func (s *Store) ListPaymentPlans(ctx context.Context, kind string, enabledOnly bool) ([]domain.PaymentPlan, error) {
	...
	defer rows.Close()
	plans := make([]domain.PaymentPlan, 0)
	for rows.Next() {
...
func (s *Store) FindPaymentPlan(ctx context.Context, id int64) (domain.PaymentPlan, error) {
	return scanPaymentPlan(s.db.QueryRowContext(ctx, paymentPlanSelect+" WHERE id = ?", id))
}

func (s *Store) UpdatePaymentPlan(ctx context.Context, id int64, input domain.UpdatePaymentPlanInput, now time.Time) (domain.PaymentPlan, error) {
	durationMinutes := input.DurationMinutes
	if durationMinutes == 0 {
		durationMinutes = input.DurationDays * 24 * 60
	}
	...
	if changed != 1 {
		return domain.PaymentPlan{}, sql.ErrNoRows
	}
	return s.FindPaymentPlan(ctx, id)
}
...
func (s *Store) DeletePaymentPlan(ctx context.Context, id int64) error {
	...
	return domain.ErrPaymentPlanInUse
}
...
func (s *Store) CreatePaymentOrder(ctx context.Context, input domain.CreatePaymentOrderInput) (domain.PaymentOrder, error) {
...
func (s *Store) FindPaymentOrder(ctx context.Context, id int64) (domain.PaymentOrder, error) {
	return s.findPaymentOrderWith(ctx, s.db, paymentOrderSelect+" WHERE id = ?", id)
}

func (s *Store) FindPaymentOrderByToken(ctx context.Context, token string) (domain.PaymentOrder, error) {
	return s.findPaymentOrderWith(ctx, s.db, paymentOrderSelect+" WHERE public_token = ?", token)
}

func (s *Store) FindPaymentOrderByMerchantNo(ctx context.Context, merchantOrderNo string) (domain.PaymentOrder, error) {
	return s.findPaymentOrderWith(ctx, s.db, paymentOrderSelect+" WHERE merchant_order_no = ?", merchantOrderNo)
}

func (s *Store) ListPendingPaymentOrders(ctx context.Context, limit int) ([]domain.PaymentOrder, error) {
	if limit < 1 {
	...
	defer rows.Close()
	orders := make([]domain.PaymentOrder, 0)
	for rows.Next() {
...
func (s *Store) ListPaymentOrders(ctx context.Context, filter domain.PaymentOrderFilter) (domain.PaymentOrderPage, error) {
	page := filter.Page
	if page < 1 {
		page = 1
	}
	...
	var result domain.PaymentOrderPage
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*), COALESCE(SUM(CASE WHEN payment_status = 'paid' THEN 1 ELSE 0 END), 0), COALESCE(SUM(CASE WHEN payment_status = 'paid' THEN amount_fen ELSE 0 END), 0) FROM payment_orders`+where, args...).Scan(&result.Total, &result.PaidCount, &result.PaidAmountFen); err != nil {
		return domain.PaymentOrderPage{}, err
	}
	result.Page = page
	...
	defer rows.Close()
	result.Orders = make([]domain.PaymentOrder, 0, pageSize)
	for rows.Next() {
		order, err := scanPaymentOrder(rows)
		if err != nil {
			return domain.PaymentOrderPage{}, err
...
func (s *Store) UpdatePaymentProvider(ctx context.Context, input domain.UpdatePaymentProviderInput) (domain.PaymentOrder, error) {
...
func (s *Store) FulfillPaymentOrder(ctx context.Context, input domain.FulfillPaymentOrderInput) (domain.PaymentOrder, error) {
	if input.OrderID < 1 || strings.TrimSpace(input.EventID) == "" || input.AmountFen < 1 || input.Currency == "" || input.PaidAt.IsZero() || input.Now.IsZero() {
		return domain.PaymentOrder{}, errors.New("invalid payment fulfillment input")
	}
	...
	if order.AmountFen != input.AmountFen || order.Currency != input.Currency {
		return domain.PaymentOrder{}, fmt.Errorf("payment amount or currency does not match local order")
	}
	...
	if eventErr == nil {
		if existingOrderID != order.ID {
			return domain.PaymentOrder{}, fmt.Errorf("payment event belongs to another order")
		}
		result, err := s.findPaymentOrderWith(ctx, conn, paymentOrderSelect+" WHERE id = ?", order.ID)
		if err != nil {
			return domain.PaymentOrder{}, err
	...
	if !errors.Is(eventErr, sql.ErrNoRows) {
		return domain.PaymentOrder{}, eventErr
	}
	if input.EventType == "" {
	...
	case "activation":
		if strings.TrimSpace(input.ActivationCode) == "" || strings.TrimSpace(input.ActivationCodeHash) == "" || strings.TrimSpace(input.ActivationCodePrefix) == "" {
			return domain.PaymentOrder{}, errors.New("activation code is required for activation fulfillment")
		}
	...
	case "renewal":
		if order.AccountID == nil {
			return domain.PaymentOrder{}, errors.New("renewal order has no account")
		}
		...
		if changed, err := result.RowsAffected(); err != nil {
			...
			return domain.PaymentOrder{}, domain.ErrAccountVersionConflict
		}
		if reactivated {
	...
	default:
		return domain.PaymentOrder{}, fmt.Errorf("unknown payment order kind %q", order.Kind)
	}
...
}, query string, args ...any) (domain.PaymentOrder, error) {
...
func scanPaymentPlan(row interface{ Scan(...any) error }) (domain.PaymentPlan, error) {
...
func scanPaymentOrder(row interface{ Scan(...any) error }) (domain.PaymentOrder, error) {

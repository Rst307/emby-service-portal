...
func (s *Store) ListAccessSyncJobs(ctx context.Context, limit int) ([]domain.AccessSyncJob, error) {
	if limit < 1 {
		return []domain.AccessSyncJob{}, nil
	}
	...
	defer rows.Close()
	jobs := make([]domain.AccessSyncJob, 0)
	for rows.Next() {
...
func scanAccessSyncJob(row accessSyncJobScanner) (domain.AccessSyncJob, error) {
	var job domain.AccessSyncJob
	...
	if err := row.Scan(&job.Account.ID, &job.Account.Version, &job.Account.EmbyUserID, &job.Account.Username, &job.Account.Status, &expires, &job.Account.Note, &created, &updated, &disabled, &desired, &job.Revision, &job.Attempts, &job.LastError); err != nil {
		return domain.AccessSyncJob{}, err
	}
	var err error
